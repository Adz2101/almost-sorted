<?php
    ...
    if (isset($_GET['category']) && isset($_GET['riskMapping'])) {
        ...
        $stmt=mysqli_prepare($conn,$sql);
        mysqli_stmt_bind_param($stmt, "ii", $mi_id, $id);
        if (mysqli_stmt_execute($stmt)) {
            // Mitigation approved successfully
            echo "Mitigation successfully planned. Redirecting...";
        header("Refresh:2; url=admin_dashboard.php"); // Redirect to dashboard after 2 seconds
